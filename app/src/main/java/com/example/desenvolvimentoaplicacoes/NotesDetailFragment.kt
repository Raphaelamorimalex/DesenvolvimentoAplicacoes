...
import com.example.desenvolvimentoaplicacoes.database.NotesProvider.Companion.URI_NOTES
...
class NotesDetailFragment : DialogFragment(), DialogInterface.OnClickListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        var newNote = true
        if(newNote != null && arguments?.getLong(EXTRA_ID) != 0L){
            ...
            if(cursor?.moveToNext()as Boolean){
                newNote = false
                noteEditTitle.setText(getString(cursor.getColumnIndex(TITLE_NOTES)))
                noteEditDescription.setText(getString(cursor.getColumnIndex(DESCRIPTION_NOTE)))
            }
            cursor.close()
        }
    ...
    override fun onClick(dialog: DialogInterface?, which: Int) {
        ...
        values.put(DESCRIPTION_NOTE, noteEditDescription.text.toString())
